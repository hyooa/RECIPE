...
<?php
        ...
        function notice(){
            ...
            while($row = mysqli_fetch_array($result2)){
                echo "<tr>
                        <td>{$row['title']}</td>
                        <td><a href=\"notice_detail.php?no={$row['no']}\">{$row['content']}</a></td>
                        <td>{$row['id']}</td>
                        <td>{$row['date']}</td>
                    </tr>";
                }
        ...
        <p class="pager">
            ...
            <?php
                ...
                <?php
                    ...
                    ?>
                    <a href="notice.php?page=<?=($page-1)?>">이전</a>
                <?php
                }
            ?>
            <?php
                for($print_page=$s_pageNum; $print_page<=$e_pageNum; $print_page++){
                    ?>
                    <a href="notice.php?page=<?=$print_page?>"><?=$print_page?></a>
                    <?php
                }
            ?>
            <!-- 다음버튼 -->
            <?php
                if($page>=$total_page){
                    ?>
                        <a href="notice.php?page=<?=$total_page?>">다음</a>
                    <?php
                }else{
                    ?>
                    <a href="notice.php?page=<?=($page+1)?>">다음</a>
                    <?php
                }
            ?>
        </p>
